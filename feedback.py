...
class ImportFeedback:
    ...
    def get_data(self):
        ...
        xlsx_file = Path('/home/chen7874/QoL1/QoL/main', 'feedback.xlsx')
        ...
        for row in sheet.iter_rows(max_row=sheet.max_row-1):
            ...
            if Baseline_Survey.objects.filter(patient_ID=self.getID(i), date_time=datetime.datetime(int(lab_year), int(lab_month), int(lab_day))):
                continue
            baseline_Survey = Baseline_Survey(patient_ID=self.getID(i),
                                              date_time=datetime.datetime(int(lab_year), int(lab_month), int(lab_day)), mobility=int(row[4].value), self_care=int(row[5].value), usual_activities=int(row[6].value), pain_level=int(row[7].value), anxiety_level=int(row[8].value), health_rating=int(row[4].value)+int(row[5].value)+int(row[6].value)+int(row[7].value)+int(row[8].value))
